...
This URL restores `filter = "active"` and `id = 123L`. `NavCenter.navigate(String)` and
`NavCenter.resolve(String)` are suspend APIs because interceptors may perform asynchronous login
checks or network operations.
